"""Env-only settings for the bridge.
...
OAuth (`CLAUDE_CONFIG_DIR`, injected into the container). We only read env.

`ROOM_ID` is **optional** (Step 2): set it → single-room mode (Step-1 back-compat); leave it empty
→ auto mode, where the bridge discovers every room the agent is a participant of (poll
`GET /chater/rooms` every `ROOMS_POLL_S`) and joins new ones as they appear.
"""
...
class Settings:
    ...
    # Optional pin to a single room. Empty → auto mode (discover every room the agent belongs to).
    room_id: str = field(default_factory=lambda: os.environ.get("ROOM_ID", ""))
    # Auto-mode discovery cadence: how often to poll GET /chater/rooms for new/removed rooms.
    rooms_poll_s: float = field(default_factory=lambda: float(os.environ.get("ROOMS_POLL_S", "10")))
    ...
    @property
    def single_room(self) -> bool:
        """True when pinned to one room (Step-1 back-compat); False → auto-discovery mode."""
        return bool(self.room_id)

    def ws_url_for(self, room_id: object) -> str:
        """ws(s):// upgrade URL for a room feed — scheme derived from `chater_url`."""
        base = self.chater_url.replace("http://", "ws://", 1).replace("https://", "wss://", 1)
        return f"{base}/chater/rooms/{room_id}/ws"
