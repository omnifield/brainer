...
На старте `ChannelHub.resume_all()` грузит реестр в память **ленивыми (cold) каналами** — БЕЗ спавна
claude-процессов. SDK-клиент поднимается только при первом обращении к сессии (`send`/`subscribe`) —
`ensure_started` (жадный resume-на-старте плодил холостые процессы на каждый рестарт). Статус при
загрузке честно реконсилится: `stopped` (персистнутый) — остаётся `stopped` и не резюмится; сессия
без `sdk_session_id` — невоскресима (`error`-событие, В2); всё прочее → `waiting` (turn не переживает
рестарт, честного «running» после него нет). `shutdown` дисконнектит все ЖИВЫЕ клиенты (нет
`<defunct>`-зомби); cold-каналы процесса не держат. В2 соблюдён: handle'ы в памяти, список их видит,
`send`/`subscribe` резюмят по требованию.
