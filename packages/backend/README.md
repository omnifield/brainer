...
Спавнабельность репо = наличие в его корне лаунчера `claude-scope.ps1` (он же маркер). Реестр
собирается **не по именам папок и не счётом родителей** (ломается при чекауте в папку с другим
именем), а так:

1. **`BRAINER_REPOS`** (env, приоритет) — явный список `name=path`, разделитель `;`. Пример:
   `BRAINER_REPOS='omnifield/brainer=/srv/brainer;omnifield/writer=/srv/writer'`.
2. **Discovery** (fallback, без env) — от `config.py` идём **вверх** до первого каталога с
   `claude-scope.ps1` (свой корень), затем сканируем его родителя: каждый sibling с лаунчером =
   managed repo. Contract-имя = `<родитель>/<каталог>` (канон-layout → `omnifield/<name>`).

TODO(architect): runtime-карта репо (devopser `registry/products.md`) может позже вытеснить
env+discovery как источник правды — вне скоупа бэка.
