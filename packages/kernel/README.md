...
| `contract.py` | Операции провайдера `AgentProvider` (`launch/send/stream/resume/stop`) — эволюция старого `backend` `IAgentProvider`; `AgentSessionHandle`; словарь permission `readonly \| standard \| trusted`; `LaunchRequest`. |
