...
## ✅ Приёмка оракула (2026-07-11) — ПРИНЯТО

Сверено по файлам: дерево чистое (img.png удалён — содержимое подтверждённо живёт
текстом в К1) · PAT-проба = точное зеркало postCreate, `$NPM_CONFIG_USERCONFIG`
корректно раскрывается ВНУТРИ контейнера (внешние одинарные кавычки sh) ·
WORKSPACE от пути скрипта с комментом · exec-бит 100755 на месте. Ниты закрыты,
из брифа остался только п.4 (HMR-eyeball user) — впишется в gateway-бриф devopser.
