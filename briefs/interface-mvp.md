...
**Vite + Solid + Vitest (jsdom), минимальные стили.** Временный-но-чистый SPA, **без
бэк-зависимости** (мок-адаптер). Обоснование: Solid — дом-дефолт (канон
`shared-policy.md §3`: «DOM/Solid — jsdom»), ляжет на будущий фреймворк-фронт; отклонение
пошло бы против канона без причины. Развилка закрыта — owner не выбирает стек. Реальный
фронт из фреймворка — позже.
